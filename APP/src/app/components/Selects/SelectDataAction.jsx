...
const SelectDataAction = ({options,option,handleChange,name,handleDelete}) => {  
  const [view, setview] = useState(false);         
  const sample = defaultVals(options,option)
    
    
  const handleChanges = (value,automatico) => {             
      handleChange(name,value,automatico)        
      setview(false)
    }     
      ...
      <div         
          ...
          {sample} 
            ...
            <div className="h-7 w-14 flex">
              <button
                ...
                type="button"
                className="h-7 w-6 font-bold border-gray-300 flex  items-center justify-center border-t border-b ">
        ...
        className="absolute z-10 -mt-2 w-40 border bg-gray-50 rounded shadow-lg p-3">      
          { options.map((it, index) => (
              ...
              onClick={()=>handleChanges(it.value,it.automatico)}>
                  {it.label}</li>                
          ))}         
          </ul>   
          }
...
export default SelectDataAction;
